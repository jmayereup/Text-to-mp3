...
app.post('/api/convert', async (req, res) => {
  const { text, modelId, voice, pushToR2 } = req.body;
  ...
  try {
    ...
    console.log(`Generating speech via OpenRouter using model: ${modelId}, voice: ${voice}...`);
    const rawAudioBuffer = await openrouter.generateSpeech(modelId, text, voice);
